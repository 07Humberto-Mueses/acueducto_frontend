...
const MatriculasPage = () => {
    ...
    const handleDeleteMatricula = async (id_matricula) => {
        const confirmation = window.confirm("¿Está seguro de que desea eliminar esta matrícula?");
        if (!confirmation) {
            return;
        }

        try {
            const response = await fetch(`http://localhost:9090/eliminar_matricula?id_matricula=${id_matricula}`, {
                method: "DELETE",
            });
            const data = await response.json();
            if (response.ok) {
                notify("Matrícula eliminada exitosamente", "success");
                fetchMatriculas();
            } else {
                notify(data.message || "Error al eliminar la matrícula", "error");
            }
        } catch (error) {
            notify("Error de conexión con el servidor", "error");
            console.error("Error:", error);
                        ...
                        {matriculas.map((item, index) => (
                            <div key={index} className="matriculasTableRowCustom">
                                ...
                                <button
                                    ...
                                    <button
                                        className="crudBtnCustom"
                                        onClick={() => handleDeleteMatricula(item.id_matricula)}
                                    >
                                        <FaTrash />
                                    </button>
                                </div>
